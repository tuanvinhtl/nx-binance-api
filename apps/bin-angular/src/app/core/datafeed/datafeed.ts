...
export class Datafeed {
  ...
  async getAllSymbols() {
    const data = await new Helper().makeApiRequest('data/v3/all/exchanges');
    let allSymbols: any[] = [];
    ...
    for (const exchange of this.configurationData.exchanges) {
      ...
      for (const leftPairPart of Object.keys(pairs)) {
        const symbols = pairs[leftPairPart].map((rightPairPart: any) => {
          const symbol = new Helper().generateSymbol(
            exchange.value,
            leftPairPart,
            rightPairPart
          );
    ...
    return {
      ...
      getBars: async (
        ...
        const query = Object.keys(urlParameters)
          ...
          .join('&');
        try {
          const data = await new Helper().makeApiRequest(
            `data/histoday?${query}`
          );
          if (
      ...
      subscribeBars: (
        ...
        new Streaming().subscribeOnStream(
          symbolInfo,
          resolution,
          onRealtimeCallback,
          subscribeUID,
      ...
      unsubscribeBars: (subscriberUID: any) => {
        ...
        new Streaming().unsubscribeFromStream(subscriberUID);
      },
    };
  }
}
